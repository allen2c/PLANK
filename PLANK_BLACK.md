...
| any → `done`      | [Completion Note](#7-completion-note) section exists    |
...
### `blocked-by` / `blocks` Symmetry

These fields MUST be kept symmetric at all times:

- If A's `blocked-by` contains B, then B's `blocks` MUST contain A.
- If A's `blocks` contains B, then B's `blocked-by` MUST contain A.
- When the blocking relationship is resolved, both sides MUST be updated.
...
## Completion Note          ← REQUIRED when status is done
...
The last entry of every work session MUST end with a `**Stopped at**:` line. This is the **exact resumption point** for the next session. It MUST be specific enough that someone with no prior context can find where to continue.
...
When status enters `done`, a Completion Note section MUST be added.
...
{One sentence: what was delivered and how it differs from the original Description, if at all.}
...
- If the final deliverable differs from the original Description, the difference MUST be stated explicitly.
...
When a ticket involves diagnosing an issue, debugging, or making a non-obvious technical decision, follow this protocol. Log each step in the Action Log.
...
1. Add an Action Log entry to every ticket touched, ending with `**Stopped at**:`.
2. Update `updated` in frontmatter of every ticket touched.
3. Update `.plank/STATUS.md` — sync the Active table.
4. If the last GC was more than 7 days ago, run the [GC Ritual](#14-gc-ritual).
...
3. If the new ticket blocks A, update `blocked-by` / `blocks` on both tickets and change A's status to `blocked`.
...
2. Create a new ticket. In its frontmatter, add: `related: [{ORIGINAL-ID}]`.
...
4. **Symmetry check** — Verify all `blocked-by` / `blocks` references are symmetric and point to existing tickets.
...
- [ ] Every `done` ticket has all Acceptance Criteria checked `[x]` and a Completion Note
- [ ] Every `cancelled` ticket has a reason in its Action Log
- [ ] Every `blocked` ticket has a non-empty `blocked-by` field
- [ ] All `blocked-by` / `blocks` references are symmetric
